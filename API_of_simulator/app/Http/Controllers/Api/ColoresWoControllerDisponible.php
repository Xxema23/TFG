<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\DB;
use Illuminate\Http\JsonResponse;

class ColoresWoControllerDisponible extends Controller
{
    public function index(): JsonResponse
    {
        $results = DB::select("
            WITH datos AS ( 
                SELECT
                    vf.wo,
                    TO_DATE(vf.fch_objetivo, 'YYYY-MM-DD') AS fch_objetivo,
                    ctb.item_code,
                    ctb.item_description AS articulo,
                    ctb.req_quantity AS consumo,
                    COALESCE(s.exist, 0) AS stock,
                    TO_DATE(ec.fecha_prometida, 'YYYY-MM-DD') AS fch_entrega,
                    ec.cant_pedida AS cantidad
                FROM
                    vision_fabricacion vf
                JOIN ctb ON vf.wo = ctb.production_order
                LEFT JOIN stocks s ON ctb.item_code = s.articulo
                LEFT JOIN entrega_compra ec ON ctb.item_code = ec.cod_articulo
            ),
            disponibilidad AS (
                SELECT
                    d.*,
                    COALESCE(d.stock, 0) - d.consumo AS disponible_stock,
                    CASE
                        WHEN COALESCE(d.stock, 0) - d.consumo >= 0 THEN COALESCE(d.stock, 0) - d.consumo
                        WHEN d.fch_entrega IS NOT NULL THEN COALESCE(d.stock, 0) - d.consumo + d.cantidad
                        ELSE COALESCE(d.stock, 0) - d.consumo
                    END AS disponible
                FROM
                    datos d
            ),
            agregado AS (
                SELECT
                    wo,
                    SUM(disponible) AS disponible_total,
                    MAX(fch_entrega) AS fch_entrega_max
                FROM
                    disponibilidad
                GROUP BY
                    wo
            ),
            clasificacion AS (
                SELECT
                    d.wo,
                    d.articulo,
                    a.disponible_total,
                    a.fch_entrega_max,
                    MAX(CASE
                        WHEN d.disponible >= 0 THEN 1
                        WHEN d.disponible < 0 AND d.fch_entrega IS NOT NULL AND (d.fch_objetivo - d.fch_entrega) >= 5 THEN 2
                        WHEN d.disponible < 0 AND d.fch_entrega IS NOT NULL AND (d.fch_objetivo - d.fch_entrega) < 5 THEN 3
                        ELSE 4
                    END) AS prioridad
                FROM
                    disponibilidad d
                JOIN agregado a ON d.wo = a.wo
                GROUP BY
                    d.wo, d.articulo, a.disponible_total, a.fch_entrega_max
            )
            SELECT
                c.wo,
                CASE c.prioridad
                    WHEN 1 THEN 'VERDE'
                    WHEN 2 THEN 'VERDE CLARO'
                    WHEN 3 THEN 'AMARILLO'
                    WHEN 4 THEN 'ROJO'
                END AS color,
                c.articulo,
                c.disponible_total AS disponible,
                CASE 
                    WHEN c.disponible_total >= 0 THEN TO_CHAR(c.fch_entrega_max, 'YYYY-MM-DD')
                    ELSE 'sp'
                END AS fecha_entrega_prometida
            FROM
                clasificacion c
            ORDER BY
                c.wo
        ");

        return response()->json($results);
    }
}
